...
class RTSPFFmpegReader:
    ...
    def __init__(
        ...
        gpu_id: int = 0,
        transport: str = "auto",  
        first_frame_timeout_s: float = 20.0,  
        stall_timeout_s: float = 8.0,
        log: bool = True,
        buffer_size: int = 10,
        probesize: int = 10_000_000, 
        analyzeduration: int = 10_000_000,
        max_delay: int = 500000,  
    ...
    def _cmd(self) -> list:
        ...
        cmd = [
            ...
            "-nostdin",
            "-hide_banner",
            "-loglevel", "info" if self.log else "error",  
        ]
...
class RTSPFFmpegWriter:
    ...
    def __init__(
        ...
        if ffmpeg_bin:
            ...
            if os.path.exists(system_ffmpeg):
                self.ffmpeg_bin = system_ffmpeg
            else:
                self.ffmpeg_bin = "ffmpeg"  
        
        self.use_gpu = self._check_nvenc_support() if use_gpu else False
        if use_gpu and not self.use_gpu:
            if self.log:
                print(f"[Writer] WARNING: GPU encode requested but h264_nvenc not available in {self.ffmpeg_bin}")
                print(f"[Writer] Falling back to CPU encode (libx264)")
                try:
                    result = subprocess.run([self.ffmpeg_bin, "-version"], capture_output=True, text=True, timeout=2)
                    first_line = result.stdout.split('\n')[0] if result.stdout else "unknown"
                    print(f"[Writer] Using: {first_line}")
                except Exception as e:
                    raise e
    ...
    def _check_nvenc_support(self) -> bool:
        """Check if FFmpeg was compiled with h264_nvenc support."""
        try:
...
def cleanup_old_images(base_path="./deploy/image/", days_to_keep=7):
